...
float randomNumber(){
   //return ((float)rand()/RAND_MAX);   /240722 by alberto
   return ((float)(rand()/RAND_MAX));   //240722 by alberto
}

Vector3 random_unit_vector() {
 //   float theta = (float)rand() / RAND_MAX * 2.0f * M_PI;	//240722 by alberto
//    float phi = (float)rand() / RAND_MAX * M_PI;			//240722 by alberto
    float theta = randomNumber() * 2.0f * M_PI;				//240722 by alberto
    float phi = randomNumber()* M_PI;						//240722 by alberto
...
float random_float(){
    //return 2*((float)rand()/RAND_MAX)-1;					//240722 by alberto
    return 2*randomNumber()-1;								//240722 by alberto
}
...
void generate_cylinder(Object *cylinder, float radius, float height, int slices) {
    int vertex_count = (slices + 1) * 2; // top and bottom vertices plus center points
    int triangle_count = slices * 2 * 3; // sides and caps
    ...
    for (int i = 0; i <= slices; ++i) {
        ...
        // Bottom circle
        vertices[idx++] = (Vector3){x, 0, z};
        // Top circle
        vertices[idx++] = (Vector3){x, height, z};
    }
    ...
    for (int i = 0; i < slices; ++i) {
        ...
        // Bottom circle
        triangles[idx++] = i * 2;
        triangles[idx++] = next * 2;
        triangles[idx++] = i * 2 + 1;

        // Top circle
        triangles[idx++] = i * 2 + 1;
        triangles[idx++] = next * 2;
        triangles[idx++] = next * 2 + 1;
    }
...
//240721 vertices y triangles intercambiados
 //   *cylinder = (Object){vertices, triangles, 0,vertex_count, idx / ,0, (Vector3){1.0f, 1.0f, 1.0f}};    
    *cylinder = (Object){triangles, vertices, 0, vertex_count, idx, 0, (Vector3){1.0f, 1.0f, 1.0f}};
}
...
int main() {
    ...
    Object sphere4;
    Object cylinder;
    generate_cylinder(&cylinder, 1,1,16);
    ...
    Object objects[] = {cylinder, plane, sphere2,sphere3,sphere4};
