...
void	check_bounds_values(t_bbox *bbox, t_vector3 v)
{
	if (v.x < bbox->min.x)
		bbox->min.x = v.x;
	if (v.y < bbox->min.y)
...
void	init_bbox_values(t_bbox *bbox)
{
	bbox->min.x = INFINITY;
	bbox->min.y = INFINITY;
	bbox->min.z = INFINITY;
...
void	setinitialbbox(t_bbox *bbox, t_global *global)
{
	int	i;
	int	j;
...
void	set_tri_val(t_triangle *tri, t_global *global, int *iter)
{
